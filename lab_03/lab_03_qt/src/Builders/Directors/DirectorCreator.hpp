...
public:
    ...
    [[nodiscard]] virtual std::shared_ptr<BaseObjectDirector> createDirector(const std::string &filename) const = 0;
};

template <typename Type>
requires std::is_base_of_v<BaseObjectDirector, Type> && (! std::is_abstract_v<Type>)
class ConcreteDirectorCreator : public DirectorCreator
{
public:
    ~ConcreteDirectorCreator() override = default;

    [[nodiscard]] std::shared_ptr<BaseObjectDirector> createDirector(const std::string &filename) const override
    {
