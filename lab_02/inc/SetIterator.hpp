...
class SetIterator : public BaseIterator
{
    public:
        ...
        SetIterator<Key>& operator =(const SetIterator<Key> &iter) { this->list_iter = iter.list_iter; };
        SetIterator<Key>& operator =(SetIterator<Key> &&iter) { this->list_iter = std::move(iter.list_iter); };
